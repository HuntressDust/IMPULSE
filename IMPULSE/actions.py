...
class MeleeAction(AttackAction):
    ...
    def perform(self) -> None:
        ...
        print(hasattr(self.entity,"equipment"))
    ...
    def perform(self) -> None:
        if self.target_actor:
            if self.target_actor.name != "doll":
                return  MeleeAction(self.entity, self.target_actor.x, self.target_actor.y).perform()
            else:
    ...
    def perform(self) -> None:
        ...
        for actor in self.engine.game_map.actors:
            ...
            if distance <= self.radius:
                ...
                self.engine.message_log.add_message(
                    f"The {actor.name} is caught in an explosion and takes {damage} damage"
                )
        ...
        for actor in self.engine.game_map.actors:
            ...
            if distance <= self.radius:
                if actor is self.engine.player:
                    self.engine.message_log.add_message(
                        f"The {self.entity.name} says something rude about your body")
                    if not actor.status.has_effects("DYS"):
                        if not actor.cyberware.has_euphoric_perk:
                            self.engine.message_log.add_message(
                            f"You suddenly feel unhappy with your physical form "
                            )
...
class ChargeAction(ActionWithDirection):
    ...
    def handle_knockback(self, target):
        knockback=False
        print("Attempting knockback calulation")
        for i in range(2):
    ...
    def perform(self) -> None:
        ...
        if self.blocking_entity is not None:
            ...
            if target_hit:
                ...
                total_damage = target.fighter.take_damage(damage)
                ...
                if target is self.engine.player:
                    self.engine.message_log.add_message(
                        f"{self.entity.name.upper()} misses {target.name}!")

                else:
                    self.engine.message_log.add_message(
                        f"{self.entity.name.upper()} misses the {target.name}!")
