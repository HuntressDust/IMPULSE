    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        for effect in consumer.status.effects:
            if effect.abrev == "SHLD":
                raise Impossible("You are already Sheilded")
...
class flareConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        self.engine.message_log.add_message(
            f"You light the flare against the {target.name} for {damage} hp!", color.status_effect
        )
        if target.fighter.burn_resist<=20:
            target.status.add_effect(status_effects.Burning(target))
            self.engine.message_log.add_message(f"The flare sets the {target.name} on fire!", color.status_effect)
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            ...
            self.engine.message_log.add_message(
                    ...
                    f"sparks fly and shocks the {target.name} for {damage}"
            )
        ...
        for actor in self.engine.game_map.actors:
            if actor.distance(*target_xy) <= self.radius:
                ...
                if actor.fighter.burn_resist < 40:
                    actor.status.add_effect(status_effects.Burning)
                    self.engine.message_log.add_message(f"The {actor.name} has been set on fire!")
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        for actor in self.engine.game_map.actors:
            if actor.distance(*target_xy) <= self.radius:
                ...
                self.engine.message_log.add_message(
                    f"The {actor.name} is caught in an explosion of fire for {damage} hit points"
                )
